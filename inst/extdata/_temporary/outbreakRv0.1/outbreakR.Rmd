---
title: "outbreakR, v. 0.1 : Insect outbreak reconstructions in R"
...
#setwd( "E:/outbreakR") # setwd to outbreakR project folder, in my case on the 'E' drive
```
...
## Determine outbreaks using set standard deviation
 ...
 if( perc == TRUE) {
   ...
   res$year <- rownames(res)
   
   res2 <- melt(res, id = "year", variable.name = "treeID", value.name = "outbreak")
   
   outbreakProp <- res2 %>%
     group_by(year) %>%
     summarize(outbreakProp = mean(outbreak) * 100)
   
   res <- merge(res, outbreakProp, by = "year")
   
   rm(res2); rm(outbreakProp)
...
## create sample CIs
MPD.ci <- corIndex( rwi = MPD.rwi, crn = PIPO.crn, scale = T)
tail(MPD.ci)
